...
router.get('/overview', async (req, res) => {
    try {
        ...
        // recordsets[1] = NC status dist (unused), recordsets[2] = machines by area
        const machinesByArea = result.recordsets[2];
        // recordsets[3] = NC monthly trend (unused), recordsets[4] = maintenance by periodicity
        ...
        try {
            const ticketResult = await pool.request()
                .input('EntityID', sql.Int, req.entityId)
                .query("SELECT COUNT(*) AS OpenCount FROM SupportTickets WHERE Status NOT IN ('CLOSED', 'CANCELLED', 'RESOLVED') AND EntityID = @EntityID");
...
router.get('/workforce', async (req, res) => {
    try {
        ...
        const kpis = result.recordsets[0][0];
        // recordsets[1] = NC workload (unused), recordsets[2] = by department
        const byDepartment = result.recordsets[2];
        const authCoverage = result.recordsets[3];
        // recordsets[4] = NC performance (unused)
